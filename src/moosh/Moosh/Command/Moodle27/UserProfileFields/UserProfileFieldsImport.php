 ...
 *
 * orange : manage strings with '\n'
 *                    + update DB if user field exists
 */
...
class UserProfileFieldsImport extends MooshCommand
{
    ...
    public function execute() {
        ...
        while(false !== $row = $csvreader->next()) {
            ...
            // Orange modification : manage strings with '\n'.
            $userfield->param1              = str_replace('\n',"\n",$row[$columnsids['param1']]);
            $userfield->param2              = str_replace('\n',"\n",$row[$columnsids['param2']]);
            $userfield->param3              = str_replace('\n',"\n",$row[$columnsids['param3']]);
            $userfield->param4              = str_replace('\n',"\n",$row[$columnsids['param4']]);
            $userfield->param5              = str_replace('\n',"\n",$row[$columnsids['param5']]);
    ...
    protected function get_or_create_userfield($userfield) {
        ...
        if ($userfieldresult) {
            // Orange modification : update DB if user field exists.
            $userfield->id = $userfieldresult->id;
            $DB->update_record('user_info_field', $userfield);
            echo "Updated: ".$userfield->shortname."\n";
        }
        else {
