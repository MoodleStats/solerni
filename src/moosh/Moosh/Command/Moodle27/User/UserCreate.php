...
class UserCreate extends MooshCommand
{
    ...
    public function execute()
    {
        ...
        foreach ($this->arguments as $argument) {
            // new Orange test : test if user exist.
            if (!$id = $DB->get_field('user', 'id', array('username' => $argument))) {
                ...
                if($options['auth'] && $options['auth'] != "manual" && !$password){
                    ...
                    // with welcome message plugin, there is a long trace.
                    $newuserid = user_create_user($user);
                }
                echo "$newuserid\n";
            }else
                echo "User already exist id=" .$id . "\n";
        }
    }
}
