...
class BlockAdd extends MooshCommand
{
    ...
    public function execute()
    {
        ...
        switch ($mode) {
            ...
            case 'system':
                ...
                $block_instances = $DB->get_records('block_instances', array('blockname'=>$blocktype, 'pagetypepattern' => $pagetypepattern, 'parentcontextid' => $context->id));
                if (!count($block_instances)) {
                    self::blockAdd($context->id,$blocktype,$pagetypepattern,$region,$weight,$showinsubcontexts);
                }else{
                    echo "Block '" . $blocktype . "' already exist on page '" . $pagetypepattern . "' with contextid " . $context->id . "\n";
                }
    ...
    {
        return "\n\nARGUMENTS:".
                "\n\tsystem 0 blocktype pagetypepattern region weight".
