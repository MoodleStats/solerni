<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'enrol_orangeinvitation'
 *
 * @package    enrol
 * @subpackage orangeinvitation
 * @copyright  Orange 2015 based on Jerome Mouneyrac invitation plugin{@link http://www.moodleitandme.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['expiredtoken'] = 'Invalid token or link is not activated.';
$string['inviteusers'] = 'Invite users';
$string['noinvitationinstanceset'] = 'No invitation enrolment instance has been found. Please add an invitation enrol instance to your course first.';
$string['nopermissiontosendinvitation'] = 'No permission to send invitations';
$string['pluginname'] = 'Invitation URL';
$string['pluginname_desc'] = 'The Invitation module allows to send invitations by email. These invitations can be used only once. Users clicking on the email link are automatically enrolled.';
$string['invitationlink'] = 'Course URL link';
$string['invitationlink_help'] = 'URL link to be provided in email invitation for that course. This link will allow to arrive directly one the course presentation page, the user can then decide to enrol or not to the course';
$string['status'] = 'Allow invitation enrolments';
$string['status_desc'] = 'Allow users to invite people to enrol into a course by an URL.';
$string['linknotactivated'] = 'This link is not activated.';