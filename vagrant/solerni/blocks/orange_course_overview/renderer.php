 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_orange_course_overview_renderer extends plugin_renderer_base {
     ...
     */
    public function orange_course_overview($courses, $overviews) {
        $html = '';
        $config = get_config('block_orange_course_overview');
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_orange_course_overview.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_orange_course_overview', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/orange_course_overview/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                    array('src' => $this->output->pix_url('movehere'),
                        'alt' => get_string('movetofirst', 'block_orange_course_overview', $courses[$movingcourseid]->fullname),
        ...
        foreach ($courses as $key => $course) {
            ...
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                        array('src' => $this->pix_url('t/move')->out(false),
                            'alt' => get_string('movecourse', 'block_orange_course_overview', $course->fullname),
            ...
            if (!empty($config->showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_orange_orange_course_overview_get_child_shortnames($course->id)) {
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                        array('src' => $this->output->pix_url('movehere'),
                            'alt' => get_string('moveafterhere', 'block_orange_course_overview', $a),
                            'title' => get_string('movehere')));
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_orange_course_overview', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_orange_course_overview'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumber', $options, block_orange_orange_course_overview_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_orange_course_overview'));
    ...
    public function hidden_courses($total) {
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            $output .= get_string('hiddencoursecount'.$plural, 'block_orange_course_overview', $total);
        } else {
            $a = new stdClass();
            $a->coursecount = $total;
            $a->showalllink = html_writer::link(new moodle_url('/my/index.php', array('mynumber' => block_orange_course_overview::SHOW_ALL_COURSES)),
                    get_string('showallcourses'));
            $output .= get_string('hiddencoursecountwithshowall'.$plural, 'block_orange_course_overview', $a);
        }
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_orange_course_overview.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_orange_course_overview', $USER->firstname));

        $plural = 's';
        if ($msgcount > 0) {
            $output .= get_string('youhavemessages', 'block_orange_course_overview', $msgcount);
            if ($msgcount == 1) {
                $plural = '';
            }
        } else {
            $output .= get_string('youhavenomessages', 'block_orange_course_overview');
        }
        $output .= html_writer::link(new moodle_url('/message/index.php'), get_string('message'.$plural, 'block_orange_course_overview'));
        $output .= $this->output->box_end();
