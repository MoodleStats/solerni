...
$numberofslides = theme_solerni_showslider();

if ($numberofslides) {
    $slideinterval  = theme_solerni_get_setting('slideinterval');
    $captionscenter = (theme_solerni_get_setting('slidecaptioncentred'))? ' centred' : '';
    $captionoptions = theme_solerni_get_setting('slidecaptionoptions');
    ...
    <div class="row-fluid">
        <div class="span12">
            <div id="essentialCarousel" class="carousel slide" data-interval="<?php echo $slideinterval;?>">
                <?php echo theme_solerni_edit_button('theme_solerni_slideshow');?>
                ...
                <div class="carousel-inner<?php echo $captionscenter.$captionsbelowclass;?>">
                    <?php for ($slideindex = 1; $slideindex <= $numberofslides; $slideindex++) {
                        echo theme_solerni_render_slide($slideindex, $captionoptions);
                    } ?>
                </div>
                <?php echo theme_solerni_render_slide_controls($left); ?>
            </div>
        </div>
    </div>
<?php } ?>