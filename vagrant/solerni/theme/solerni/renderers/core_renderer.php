 ...
 */
class theme_solerni_core_renderer extends core_renderer
{
    ...
    {
        $breadcrumbstyle = theme_solerni_get_setting('breadcrumbstyle');
        if ($breadcrumbstyle) {
    ...
    public function footer()
    {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = theme_solerni_performance_output($perf, theme_solerni_get_setting('perfinfo'));
            }
        }
    ...
    public function custom_menu_courses()
    {
        ...
        $hasdisplaymycourses = theme_solerni_get_setting('displaymycourses');
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            $mycoursetitle = theme_solerni_get_setting('mycoursetitle');
            if ($mycoursetitle == 'module') {
                $branchtitle = get_string('mymodules', 'theme_solerni');
            } else if ($mycoursetitle == 'unit') {
                $branchtitle = get_string('myunits', 'theme_solerni');
            } else if ($mycoursetitle == 'class') {
                $branchtitle = get_string('myclasses', 'theme_solerni');
            } else {
                $branchtitle = get_string('mycourses', 'theme_solerni');
            }
            ...
            if ($numcourses == 0 || empty($courses)) {
                $noenrolments = get_string('noenrolments', 'theme_solerni');
    ...
    public function custom_menu_themecolours()
    {
        ...
        if (!isguestuser()) {
            ...
            foreach (range(1, 3) as $alternativethemenumber) {
                if (theme_solerni_get_setting('enablealternativethemecolors' . $alternativethemenumber)) {
                    $alternativethemes[] = $alternativethemenumber;
                }
            }
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_solerni');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl = new moodle_url('#');
                $branchsort = 300;
                $branch = $colourmenu->add($branchlabel, $branchurl, $branchtitle, $branchsort);

                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_solerni');
                ...
                foreach ($alternativethemes as $alternativethemenumber) {
                    if (theme_solerni_get_setting('alternativethemename' . $alternativethemenumber)) {
                        $alternativethemeslabel = theme_solerni_get_setting('alternativethemename' . $alternativethemenumber);
                    } else {
                        $alternativethemeslabel = get_string('alternativecolors', 'theme_solerni', $alternativethemenumber);
                    }
    ...
    public function custom_menu_messages()
    {
        ...
        if (empty($totalmessages)) {
            ...
            $messagetitle = get_string('nomessagesfound', 'theme_solerni');
    ...
    private function get_time_difference($created_time)
    {
        ...
        if (intval($time_difference / $years) > 1) {
            ...
            return get_string('ago', 'core_message', get_string('few', 'theme_solerni') . get_string('seconds'));
        }
    }
    ...
    public function custom_menu_goto_bottom()
    {
        ...
        if (($this->page->pagelayout == 'course') || ($this->page->pagelayout == 'incourse') || ($this->page->pagelayout == 'admin')) { // Go to bottom.
            ...
            $menu->add($gotobottom, new moodle_url('#region-main'), get_string('gotobottom', 'theme_solerni'));
    ...
    public function custom_menu_user()
    {
        ...
        if (!isloggedin()) {
            ...
            // Render Help Link
            $usermenu .= $this->theme_solerni_render_helplink();
            ...
            if (\core\session\manager::is_loggedinas()) {
                ...
                $branchlabel = '<em><i class="fa fa-key"></i>' . fullname($realuser, true) . get_string('loggedinas', 'theme_solerni') . fullname($USER, true) . '</em>';
            ...
            if (is_mnet_remote_user($USER) && $idprovider = $DB->get_record('mnet_host', array('id' => $USER->mnethostid))) {
                $branchlabel = '<em><i class="fa fa-users"></i>' . get_string('loggedinfrom', 'theme_solerni') . $idprovider->name . '</em>';
            ...
            $usermenu .= $this->theme_solerni_render_preferences($context);
            ...
            if ($course->id == 1) {
                ...
                foreach ($hascourses as $hascourse) {
                    ...
                    if (has_capability('gradereport/user:view', $reportcontext) && $hascourse->visible) {
                        $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('mygrades', 'theme_solerni') . '</em>';
                ...
                $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('coursegrades', 'theme_solerni') . '</em>';
     ...
     * @return string
     */
    private function theme_solerni_render_helplink()
    {
        global $USER, $CFG;
        if (!theme_solerni_get_setting('helplinktype')) {
            return false;
        }
        ...
        if (theme_solerni_get_setting('helplinktype') === '1') {
            if (theme_solerni_get_setting('helplink') && filter_var(theme_solerni_get_setting('helplink'), FILTER_VALIDATE_EMAIL)) {
                $branchurl = 'mailto:' . theme_solerni_get_setting('helplink') . '?cc=' . $USER->email;
                ...
                if (is_siteadmin()) {
                    $branchurl = preg_replace("(https?:)", "", $CFG->wwwroot).'/admin/settings.php?section=theme_solerni_header';
                }
                $branchlabel = '<em><i class="fa fa-exclamation-triangle red"></i>' . get_string('invalidemail') . '</em>';
            }
        }

        if (theme_solerni_get_setting('helplinktype') === '2') {
            if (theme_solerni_get_setting('helplink') && filter_var(theme_solerni_get_setting('helplink'), FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED)) {
                $branchurl = theme_solerni_get_setting('helplink');
                $target = '_blank';
            } else if ((!theme_solerni_get_setting('helplink')) && (filter_var($CFG->supportpage, FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED))) {
     ...
     */
    private function theme_solerni_render_preferences($context)
    {
        global $USER, $CFG;
        ...
        if (has_capability('moodle/user:editownmessageprofile', $context)) {
            $branchlabel = '<em><i class="fa fa-comments"></i>' . get_string('messagepreferences', 'theme_solerni') . '</em>';
            $branchurl = new moodle_url('/message/edit.php', array('id' => $USER->id));
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enableblogs) {
            $branchlabel = '<em><i class="fa fa-rss-square"></i>' . get_string('blogpreferences', 'theme_solerni') . '</em>';
            $branchurl = new moodle_url('/blog/preferences.php');
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enablebadges && has_capability('moodle/badges:manageownbadges', $context)) {
            $branchlabel = '<em><i class="fa fa-certificate"></i>' . get_string('badgepreferences', 'theme_solerni') . '</em>';
    ...
    */
    ...
    {
        if (theme_solerni_get_setting($socialnetwork)) {
            $icon = $socialnetwork;
            ...
            $socialhtml .= html_writer::start_tag('button', array('type' => "button",
                ...
                'onclick' => "window.open('" . theme_solerni_get_setting($socialnetwork) . "')",
                'title' => get_string($socialnetwork, 'theme_solerni'),
            ));
    ...
    public function solerni_header() {
