...
class forumng_object {
     ...
     */
    public function get_posts_by_user($userid, array $courses,
            $musthaveaccess = false, $discussionsonly = false,
            $limitfrom = 0, $limitnum = 50) {
        global $DB, $USER, $CFG;
            ...
            if ($iscurrentuser || $hascapsonuser) {
                ...
                if (groups_get_course_groupmode($course) == SEPARATEGROUPS
                  ...
                  && !has_capability('moodle/site:accessallgroups', $coursecontext)
                    ...
                    $mygroups = array_keys(groups_get_all_groups(
                            $course->id,
                            $USER->id,
                            $course->defaultgroupingid,
                            'g.id, g.name'));
                    // Get the groups the requested user is a member of.
                    $usergroups = array_keys(groups_get_all_groups(
                            $course->id,
                            $user->id,
                            $course->defaultgroupingid,
                            'g.id, g.name'));
        ...
        foreach ($return->courses as $course) {
            ...
            foreach ($modinfo->get_instances_of('forumng') as $forumid => $cm) {
                ...
                if (!has_capability('mod/forumng:viewdiscussion', $cm->context)
                        && !($hascapsonuser
                        && has_capability('mod/forumng:viewdiscussion', $cm->context, $user->id))) {
                    continue;
                }
                ...
                if (!$iscurrentuser && !$hascapsonuser) {
                    ...
                    if (!empty($CFG->forum_enabletimedposts)
                        ...
                        $forumsearchselect[] = "(d.userid = :userid{$forumid} "
                        . "OR (d.timestart < :timestart{$forumid} "
                        . "AND (d.timeend = 0 OR d.timeend > :timeend{$forumid})))";
                    ...
                    // Qanda access.
                    if ($forum->type == 'qanda'
                            && !has_capability('mod/forum:viewqandawithoutposting', $cm->context)) {
        ...
        foreach ($return->posts as $post) {
            if (!array_key_exists($post->discussionid, $listdiscussionname)) {
                ...
                $sqldiscusname = "select subject from {forumng_posts}"
                        . " where discussionid = "
                        . $post->discussionid . " AND parentpostid is null";
    ...
    /**
     * Get the posts for a course ordered by date modified
     * @return array $usedbadges
     */
    public function get_recent_posts($courseid, $limitfrom = 0, $limitnum = 50) {
        global $DB;
        ...
        // Prepare SQL to both count and search.
        $selectsql = 'SELECT p.*, d.forumngid ';
        ...
        // Set the collection of posts that has been requested.
        $return->posts = $DB->get_records_sql($selectsql.$sql.$orderby, null, $limitfrom, $limitnum);
        ...
        foreach ($return->posts as $post) {
            if (!array_key_exists($post->discussionid, $listdiscussionname)) {
                // Add discusionname before return.
                $sqldiscusname = "select subject from {forumng_posts}"
                        . " where discussionid = " . $post->discussionid
    ...
    public function get_recent_discussions($courseid, $limitfrom = 0, $limitnum = 50) {
        ...
        // Prepare SQL to both count and search.
        $selectsql = 'SELECT p.discussionid, max(p.modified) as datemodified, d.lastpostid ';
        $sql = "FROM {forumng_posts} p
                JOIN {forumng_discussions} d ON d.id = p.discussionid
                JOIN {forumng} f ON f.id = d.forumngid";
        $sql .= " WHERE p.deleted=0
                  AND d.deleted=0
                  AND f.course = $courseid";
        $sql .= " GROUP BY p.discussionid ";
        $orderby = " ORDER BY datemodified DESC";
        ...
        list($insql, $params) = $DB->get_in_or_equal($laspostidbydiscussionid);

        $sql = "SELECT p.*, d.forumngid
                FROM {forumng_posts} p
                JOIN {forumng_discussions} d ON d.id = p.discussionid
                WHERE p.id $insql
                ORDER BY p.modified DESC";

        $return->posts = $DB->get_records_sql($sql, $params);
    ...
    /**
     * Get the bests posts for a course ordered by rating
     * @return stdClass use $return->posts to access at posts
     */
    public function get_best_post($courseid, $limitfrom = 0, $limitnum = 50) {
        global $DB;
        ...
        foreach ($resps as $resp) {
            $cm = get_coursemodule_from_instance('forumng', $resp->id);
            $cms[] = $cm->id;
        }

        // Get context id.
        $sql = "SELECT id FROM {context} WHERE instanceid in (" . implode(", ", $cms) . ")";
        ...
        $selectsql = 'SELECT p.*, d.forumngid, ra.itemid as postid, SUM(ra.rating) as rate';

        $sql = " FROM {rating} ra
                JOIN {forumng_posts} p ON p.id = ra.itemid
                JOIN {forumng_discussions} d ON d.id = p.discussionid
                JOIN {forumng} f ON f.id = d.forumngid
                WHERE ra.contextid in (" . implode(", ", $contextids) . ")
                AND ra.component='mod_forumng' AND ra.ratingarea='post'
                AND p.deleted=0
                AND d.deleted=0
                GROUP BY ra.itemid";

        $orderby = " ORDER BY rate DESC, ra.timemodified DESC";
     ...
     */
    public function get_forums_user_posted_in($userid, array $courseids = null,
