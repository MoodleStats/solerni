 ...
 */
function theme_solerni_set_fontwww($css) {
    global $CFG;
    $fontwww = preg_replace("(https?:)", "", $CFG->wwwroot . '/theme/essential/fonts/');

    $tag = '[[setting:fontwww]]';

    if (theme_solerni_get_setting('bootstrapcdn')) {
...
function theme_solerni_get_setting($setting, $format = false) {
    global $CFG;
...
function theme_solerni_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    if (!($logo)) {
        $replacement = 'none';
    } else {
...
function theme_solerni_get_title($location) {
    global $CFG, $SITE;
    $title = '';
    if ($location === 'navbar') {
        $url = preg_replace("(https?:)", "", $CFG->wwwroot);
        switch (theme_solerni_get_setting('navbartitle')) {
            case 0:
                return false;
                break;
            case 1:
        ...
        switch (theme_solerni_get_setting('headertitle')) {
            case 0:
                return false;
                break;
            case 1:
...
function theme_solerni_edit_button($section) {
    global $PAGE, $CFG;
...
function theme_solerni_get_csswww() {
    global $CFG;
    ...
    $url = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/theme_solerni/style/$itemid/$moodlecss");
...
function theme_solerni_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    ...
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        if ($filearea === 'logo') {
            ...
            theme_solerni_serve_css($args[1]);
...
function theme_solerni_serve_css($filename) {
    global $CFG;
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_solerni_send_unmodified($lastmodified, $etagfile);
    }
    theme_solerni_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}

function theme_solerni_send_unmodified($lastmodified, $etag) {
...
function theme_solerni_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 * @return string
 */
function theme_solerni_set_pagewidth($css, $pagewidth) {
 ...
 * @return string $html
 */
function theme_solerni_performance_output($param, $perfinfo) {
    ...
    $html .= html_writer::tag('h2', get_string('perfinfoheading', 'theme_solerni'));
    ...
    if ($colcount != 0) {
        ...
        if (isset($param['realtime'])) {
            ...
            $html .= html_writer::span(get_string('loadtime', 'theme_solerni'));
            $html .= html_writer::end_tag('div');
        }
        if (isset($param['memory_total'])) {
            $html .= html_writer::start_tag('div', array('class' => 'span' . $thespan));
            $html .= html_writer::tag('var', display_size($param['memory_total']), array('id' => 'memory'));
            $html .= html_writer::span(get_string('memused', 'theme_solerni'));
            $html .= html_writer::end_tag('div');
        }
        if (isset($param['includecount'])) {
            $html .= html_writer::start_tag('div', array('class' => 'span' . $thespan));
            $html .= html_writer::tag('var', $param['includecount'], array('id' => 'included'));
            $html .= html_writer::span(get_string('included', 'theme_solerni'));
            $html .= html_writer::end_tag('div');
        }
        if (isset($param['dbqueries'])) {
            $html .= html_writer::start_tag('div', array('class' => 'span' . $thespan));
            $html .= html_writer::tag('var', $param['dbqueries'], array('id' => 'dbqueries'));
            $html .= html_writer::span(get_string('dbqueries', 'theme_solerni'));
            $html .= html_writer::end_tag('div');
        }
    }
    $html .= html_writer::end_tag('div');
    if ($perfinfo === "max") {
        ...
        $html .= html_writer::start_tag('div', array('class' => 'row-fluid'));
        $html .= html_writer::start_tag('div', array('class' => 'span12'));
        $html .= html_writer::tag('h2', get_string('extperfinfoheading', 'theme_solerni'));
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
        $html .= html_writer::start_tag('div', array('class' => 'row-fluid'));
        $colcountmax = 0;
        ...
        if ($colcountmax != 0) {
            ...
            if (isset($param['serverload'])) {
                ...
                $html .= html_writer::span(get_string('serverload', 'theme_solerni'));
                $html .= html_writer::end_tag('div');
            }
            if (isset($param['memory_peak'])) {
                $html .= html_writer::start_tag('div', array('class' => 'span' . $thespanmax));
                $html .= html_writer::tag('var', display_size($param['memory_peak']), array('id' => 'peakmemory'));
                $html .= html_writer::span(get_string('peakmem', 'theme_solerni'));
                $html .= html_writer::end_tag('div');
            }
            if (isset($param['cachesused'])) {
                $html .= html_writer::start_tag('div', array('class' => 'span' . $thespanmax));
                $html .= html_writer::tag('var', $param['cachesused'], array('id' => 'cache'));
                $html .= html_writer::span(get_string('cachesused', 'theme_solerni'));
                $html .= html_writer::end_tag('div');
            }
            if (isset($param['sessionsize'])) {
                $html .= html_writer::start_tag('div', array('class' => 'span' . $thespanmax));
                $html .= html_writer::tag('var', $param['sessionsize'], array('id' => 'session'));
                $html .= html_writer::span(get_string('sessionsize', 'theme_solerni'));
                $html .= html_writer::end_tag('div');
            }
            if (isset($param['dbtime'])) {
                $html .= html_writer::start_tag('div', array('class' => 'span' . $thespanmax));
                $html .= html_writer::tag('var', $param['dbtime'], array('id' => 'dbtime'));
                $html .= html_writer::span(get_string('dbtime', 'theme_solerni'));
                $html .= html_writer::end_tag('div');
            }
        }
        $html .= html_writer::end_tag('div');
...
function theme_solerni_hex2rgba($hex, $opacity) {
    $hex = str_replace("#", "", $hex);
 ...
 */
function theme_solerni_set_customcss($css, $customcss) {
...
function theme_solerni_process_css($css, $theme) {
    // Set the theme width
    $pagewidth = theme_solerni_get_setting('pagewidth');
    $css = theme_solerni_set_pagewidth($css, $pagewidth);

    // Set the theme font
    $headingfont = theme_solerni_get_setting('fontnameheading');
    $bodyfont = theme_solerni_get_setting('fontnamebody');

    $css = theme_solerni_set_headingfont($css, $headingfont);
    $css = theme_solerni_set_bodyfont($css, $bodyfont);
    $css = theme_solerni_set_fontfiles($css, 'heading', $headingfont, $theme);
    $css = theme_solerni_set_fontfiles($css, 'body', $bodyfont, $theme);

    // Set the theme colour.
    $themecolor = theme_solerni_get_setting('themecolor');
    $css = theme_solerni_set_color($css, $themecolor, '[[setting:themecolor]]', '#30ADD1');

    // Set the theme text colour.
    $themetextcolor = theme_solerni_get_setting('themetextcolor');
    $css = theme_solerni_set_color($css, $themetextcolor, '[[setting:themetextcolor]]', '#047797');

    // Set the theme url colour.
    $themeurlcolor = theme_solerni_get_setting('themeurlcolor');
    $css = theme_solerni_set_color($css, $themeurlcolor, '[[setting:themeurlcolor]]', '#FF5034');

    // Set the theme hover colour.
    $themehovercolor = theme_solerni_get_setting('themehovercolor');
    $css = theme_solerni_set_color($css, $themehovercolor, '[[setting:themehovercolor]]', '#F32100');

    // Set the theme icon colour.
    $themeiconcolor = theme_solerni_get_setting('themeiconcolor');
    $css = theme_solerni_set_color($css, $themeiconcolor, '[[setting:themeiconcolor]]', '#30ADD1');

    // Set the theme navigation colour.
    $themenavcolor = theme_solerni_get_setting('themenavcolor');
    $css = theme_solerni_set_color($css, $themenavcolor, '[[setting:themenavcolor]]', '#ffffff');

    // Set the footer colour.
    $footercolor = theme_solerni_hex2rgba(theme_solerni_get_setting('footercolor'), '0.95');
    $css = theme_solerni_set_color($css, $footercolor, '[[setting:footercolor]]', '#555555');

    // Set the footer text color.
    $footertextcolor = theme_solerni_get_setting('footertextcolor');
    $css = theme_solerni_set_color($css, $footertextcolor, '[[setting:footertextcolor]]', '#bbbbbb');

    // Set the footer heading colour.
    $footerheadingcolor = theme_solerni_get_setting('footerheadingcolor');
    $css = theme_solerni_set_color($css, $footerheadingcolor, '[[setting:footerheadingcolor]]', '#cccccc');

    // Set the footer separator colour.
    $footersepcolor = theme_solerni_get_setting('footersepcolor');
    $css = theme_solerni_set_color($css, $footersepcolor, '[[setting:footersepcolor]]', '#313131');

    // Set the footer URL color.
    $footerurlcolor = theme_solerni_get_setting('footerurlcolor');
    $css = theme_solerni_set_color($css, $footerurlcolor, '[[setting:footerurlcolor]]', '#217a94');

    // Set the footer hover colour.
    $footerhovercolor = theme_solerni_get_setting('footerhovercolor');
    $css = theme_solerni_set_color($css, $footerhovercolor, '[[setting:footerhovercolor]]', '#30add1');

    // Set the slide background colour.
    $slidebgcolor = theme_solerni_hex2rgba(theme_solerni_get_setting('themecolor'), '.75');
    $css = theme_solerni_set_color($css, $slidebgcolor, '[[setting:carouselcolor]]', '#30add1');

    // Set the slide active pip colour.
    $slidebgcolor = theme_solerni_hex2rgba(theme_solerni_get_setting('themecolor'), '.25');
    $css = theme_solerni_set_color($css, $slidebgcolor, '[[setting:carouselactivecolor]]', '#30add1');

    // Set the slide header colour.
    $slideshowcolor = theme_solerni_get_setting('slideshowcolor');
    $css = theme_solerni_set_color($css, $slideshowcolor, '[[setting:slideshowcolor]]', '#30add1');

    // Set the slide header colour.
    $slideheadercolor = theme_solerni_get_setting('slideheadercolor');
    $css = theme_solerni_set_color($css, $slideheadercolor, '[[setting:slideheadercolor]]', '#30add1');

    // Set the slide text colour.
    $slidecolor = theme_solerni_get_setting('slidecolor');
    $css = theme_solerni_set_color($css, $slidecolor, '[[setting:slidecolor]]', '#ffffff');

    // Set the slide button colour.
    $slidebuttoncolor = theme_solerni_get_setting('slidebuttoncolor');
    $css = theme_solerni_set_color($css, $slidebuttoncolor, '[[setting:slidebuttoncolor]]', '#30add1');

    // Set the slide button hover colour.
    $slidebuttonhcolor = theme_solerni_get_setting('slidebuttonhovercolor');
    $css = theme_solerni_set_color($css, $slidebuttonhcolor, '[[setting:slidebuttonhovercolor]]', '#217a94');

    if ((get_config('theme_solerni', 'enablealternativethemecolors1')) ||
            (get_config('theme_solerni', 'enablealternativethemecolors2')) ||
            (get_config('theme_solerni', 'enablealternativethemecolors3'))
    ) {
        ...
        foreach (range(1, 3) as $alternative) {
            ...
            $css = theme_solerni_set_alternativecolor($css, 'color' . $alternative,
                    theme_solerni_get_setting('alternativethemehovercolor' . $alternative), $default);
            $css = theme_solerni_set_alternativecolor($css, 'textcolor' . $alternative,
                    theme_solerni_get_setting('alternativethemetextcolor' . $alternative), $default);
            $css = theme_solerni_set_alternativecolor($css, 'urlcolor' . $alternative,
                    theme_solerni_get_setting('alternativethemeurlcolor' . $alternative), $default);
            $css = theme_solerni_set_alternativecolor($css, 'hovercolor' . $alternative,
                    theme_solerni_get_setting('alternativethemehovercolor' . $alternative), $defaulthover);
        }
    }

    // Set custom CSS.
    $customcss = theme_solerni_get_setting('customcss');
    $css = theme_solerni_set_customcss($css, $customcss);

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_solerni_set_logo($css, $logo);

    // Set the background image for the page.
    $pagebackground = $theme->setting_file_url('pagebackground', 'pagebackground');
    $css = theme_solerni_set_pagebackground($css, $pagebackground);

    // Set the background style for the page.
    $pagebgstyle = theme_solerni_get_setting('pagebackgroundstyle');
    $css = theme_solerni_set_pagebackgroundstyle($css, $pagebgstyle);

    // Set Marketing Image Height.
    $marketingheight = theme_solerni_get_setting('marketingheight');
    $css = theme_solerni_set_marketingheight($css, $marketingheight);

    // Set Marketing Images.
    $setting = 'marketing1image';
    $marketingimage = $theme->setting_file_url($setting, $setting);
    $css = theme_solerni_set_marketingimage($css, $marketingimage, $setting);
    ...
    $css = theme_solerni_set_fontwww($css);
 ...
 */
function theme_solerni_initialise_colourswitcher(moodle_page $page) {
    user_preference_allow_ajax_update('theme_solerni_colours', PARAM_ALPHANUM);
    $page->requires->yui_module(
            'moodle-theme_solerni-coloursswitcher', 'M.theme_solerni.initColoursSwitcher',
 ...
 */
function theme_solerni_get_colours($default = 'default') {
    $preference = get_user_preferences('theme_solerni_colours', $default);
    foreach (range(1, 3) as $alternativethemenumber) {
        if ($preference == 'alternative' . $alternativethemenumber && theme_solerni_get_setting('enablealternativethemecolors' . $alternativethemenumber)) {
            return $preference;
        }
    }
    return $default;
 ...
 */
function theme_solerni_check_colours_switch() {
    ...
    if (in_array($colours, array('default', 'alternative1', 'alternative2', 'alternative3'))) {
        set_user_preference('theme_solerni_colours', $colours);
    }
}

function theme_solerni_set_headingfont($css, $headingfont) {
...
function theme_solerni_set_bodyfont($css, $bodyfont) {
...
function theme_solerni_set_fontfiles($css, $type, $fontname, $theme) {
    $tag = '[[setting:fontfiles' . $type . ']]';
    $replacement = '';
    if (theme_solerni_get_setting('fontselect') === '3') {
        $fontfiles = array();
...
function theme_solerni_set_color($css, $themecolor, $tag, $default) {
    if (!($themecolor)) {
        $replacement = $default;
    } else {
...
function theme_solerni_set_alternativecolor($css, $type, $customcolor, $defaultcolor) {
...
function theme_solerni_set_pagebackground($css, $pagebackground) {
...
function theme_solerni_set_pagebackgroundstyle($css, $style) {
...
function theme_solerni_set_marketingheight($css, $marketingheight) {
...
function theme_solerni_set_marketingimage($css, $marketingimage, $setting) {
    $tag = '[[setting:' . $setting . ']]';
    if (!($marketingimage)) {
        $replacement = 'none';
    } else {
...
function theme_solerni_showslider() {
    global $CFG;
    $noslides = theme_solerni_get_setting('numberofslides');
    if ($noslides && (intval($CFG->version) >= 2013111800)) {
        $devicetype = core_useragent::get_device_type(); // In moodlelib.php.
        if (($devicetype == "mobile") && theme_solerni_get_setting('hideonphone')) {
            $noslides = false;
        } else if (($devicetype == "tablet") && theme_solerni_get_setting('hideontablet')) {
            $noslides = false;
        }
    }
    return $noslides;
...
function theme_solerni_get_nav_links($course, $sections, $sectionno) {
    ...
    while ($back > 0 and empty($links['previous'])) {
        if ($canviewhidden || $sections[$back]->uservisible) {
            ...
            $previouslink .= get_string('previoussection', 'theme_solerni');
    ...
    while ($forward <= $course->numsections and empty($links['next'])) {
        if ($canviewhidden || $sections[$forward]->uservisible) {
            ...
            $nextlink .= get_string('nextsection', 'theme_solerni');
...
function theme_solerni_print_single_section_page(&$that, &$courserenderer, $course, $sections, $mods, $modnames, $modnamesused,
        $displaysection) {
    global $PAGE;
...
function theme_solerni_render_slide($i, $captionoptions) {
    global $PAGE, $OUTPUT;

    $slideurl = theme_solerni_get_setting('slide' . $i . 'url');
    $slideurltarget = theme_solerni_get_setting('slide' . $i . 'target');
    $slidetitle = theme_solerni_get_setting('slide' . $i, true);
    $slidecaption = theme_solerni_get_setting('slide' . $i . 'caption', true);
    if ($captionoptions == 0) {
    ...
    $slideimagealt = strip_tags(theme_solerni_get_setting('slide' . $i, true));
    $slideimage = $OUTPUT->pix_url('default_slide', 'theme');

    // Get slide image or fallback to default
    if (theme_solerni_get_setting('slide' . $i . 'image')) {
...
function theme_solerni_render_slide_controls($left) {
    $faleft = 'left';
    $faright = 'right';
    if (!$left) {
        $temp = $faleft;
 ...
 */
function theme_solerni_not_lte_ie9() {
    $properties = theme_solerni_ie_properties();
 ...
 */
function theme_solerni_ie_properties() {
...
function theme_solerni_page_init(moodle_page $page) {
    ...
    if ((is_array($properties)) && ($properties['version'] <= 8.0)) {
        $page->requires->jquery_plugin('html5shiv', 'theme_solerni');
    }
    $page->requires->jquery_plugin('bootstrap', 'theme_solerni');
    $page->requires->jquery_plugin('breadcrumb', 'theme_solerni');
    $page->requires->jquery_plugin('fitvids', 'theme_solerni');
    $page->requires->jquery_plugin('antigravity', 'theme_solerni');
}
